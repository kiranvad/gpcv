...
figure(1); clf;
[c,h] = contour(reshape(x_gnd(:,1), x_shape), reshape(x_gnd(:,2), x_shape), reshape(y_gnd, x_shape), ...
    linspace(-1, 3, 5));
clabel(c,h);
title('Actual contour');

% ----------------------- aas ----------------------------
aas = ActiveAreaSearch(gp_model, gp_para,x_gnd, [0,1,0,1], level, 1, .8);
...
pause;
% ----------------------- lse ----------------------------
lse = ActiveLevelSetEstimation(gp_model, gp_para, x_gnd, level, 9, 0.1);
